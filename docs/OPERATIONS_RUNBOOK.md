...
2. Check `session_conflict_count` and `runtime_dropped_queue_overflow` metrics.
...
2. Check `transport_retries` and `telegram_api_latency_ms` p95.
...
2. `telegram_api_latency_ms` p95 back within SLO.
3. `runtime_dropped_queue_overflow` returns to baseline.
4. Alert noise resolved and dashboards stable.
