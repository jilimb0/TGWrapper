...
try {
  const projectName = options.projectName || await rl.question('Project name: ');
  const templateName = options.template || await askTemplate();
  const template = templates[templateName];

  if (!projectName) fail('Project name is required.');
  if (!template) fail(`Unknown template "${templateName}". Use standard, support, or migration.`);
  ...
  copyIfExists(join(templateRoot, 'tsconfig.json'), join(targetDir, 'tsconfig.json'));
  copyIfExists(join(templateRoot, '.env.example'), join(targetDir, '.env.example'));
  ...
  writeFileSync(join(targetDir, 'package.json'), `${JSON.stringify(generatedPackage, null, 2)}\n`);

  console.log(`Created ${projectName} from ${template.packageName}`);
  console.log(`Next: cd ${projectName} && cp .env.example .env && pnpm install && pnpm start`);
} finally {
  rl.close();
}
...
function toPackageName(value) {
  return value.toLowerCase().replace(/[^a-z0-9._-]+/g, '-').replace(/^-+|-+$/g, '') || 'tgwrapper-bot';
}
